#include "CModelOnlineMusicTopName.h"
...
namespace QTDUDU {
    ...
    {
    public:
        CModelOnlineMusicTopNamePrivate(CModelOnlineMusicTopName* model):
            m_model(model)
        {
        ...
        CModelOnlineMusicTopName*                       m_model;
        QHash<int, QByteArray>                          m_roleNames;
        QVector<HData_OnlineMusicTopnameMusicData>      m_datas;
    };

    CModelOnlineMusicTopName::CModelOnlineMusicTopName(QObject *parent) :
        QAbstractListModel(parent),
        m_dPtr(new CModelOnlineMusicTopNamePrivate(this))
    {
        qRegisterMetaType<QVector<HData_OnlineMusicTopnameMusicData>>("QVector<HData_OnlineMusicTopnameMusicData>");

        connect(CAppOnlineMusic::getInstance(), SIGNAL(sigForUpdateOnlineMusicTopnameMusicData(QVector<HData_OnlineMusicTopnameMusicData>)),
        this, SLOT(slotForOnlineMusicTopName(QVector<HData_OnlineMusicTopnameMusicData>)));
    }

    CModelOnlineMusicTopName::~CModelOnlineMusicTopName()
    {
        if (NULL != m_dPtr) {
            delete m_dPtr;
            m_dPtr = NULL;
    ...
    int CModelOnlineMusicTopName::rowCount(const QModelIndex &parent) const
    {
        Q_UNUSED(parent)
        return m_dPtr->m_datas.size();
    }

    QVariant CModelOnlineMusicTopName::data(const QModelIndex &index, int role) const
    {
    ...
    QHash<int, QByteArray> CModelOnlineMusicTopName::roleNames() const
    {
        return m_dPtr->m_roleNames;
    }

    void CModelOnlineMusicTopName::slotForOnlineMusicTopName(const QVector<HData_OnlineMusicTopnameMusicData> &data)
    {
        beginResetModel();
        m_dPtr->m_datas = data;
        endResetModel();
